...
(br,br_err,ver,ver_err,ne,ne_err,residual,seminorm,reg_corner,nes,
    ...
    epoch=epoch, stripe=stripe, l1_rev='v03r002', anc_rev='v03r001',
...
#     vers,br_orig,br_err_orig,h,A)= custom_l25(date=date, method=method,
#     epoch=epoch, stripe=stripe, l1_rev='v03r004', anc_rev='v03r000',
...
# param_list
# for i in [0,25,46,50,75,-1]:
i = 46
reg_ratio = params[i] / reg_corner
fig, ax = plt.subplots(ncols=3, figsize=(14,4.8))
ax[0].plot(ver, h, label='L-Curve Param')
ax[0].plot(vers[i], h, label='Fixed Param')
...
ax[1].plot(ne, h, label='L-Curve Param')
ax[1].plot(nes[i], h, label='Fixed Param')
...
ax[2].scatter(np.log(residual[reg_der_ind]), np.log(seminorm[reg_der_ind]), color='b', label='Automated Selection')
ax[2].scatter(np.log(residual[46]), np.log(seminorm[46]), color='orange', label='Fixed Selection')
ax[2].legend()
