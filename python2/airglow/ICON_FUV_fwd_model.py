...
# ICON FUV
def calc_1356_nighttime(lat,lon,alt,dn,Ne_scaling = 1.,testing=0,f107=None, f107a=None, f107p=None, apmsis=None):
    '''
...
# ICON FUV
def calculate_pixel_1356_nighttime(ze,az,satlat,satlon,satalt,dn,cont=1,Ne_scaling=1., step=10. , testing = 0,f107=None, f107a=None, f107p=None, apmsis=None):
    '''
    Step along a desired look direction from a given observing location and
    calculate the 135.6-nm intensity. Lines-of-sight are calculated using a spherical earth.
    INPUTS:
    ...
    while (np.sqrt(x**2 + y**2) <= RE+satalt): # until you get far enough away from earth that it doesn't matter
        ...
        VER,MN,Ne,_ = calc_1356_nighttime(satlat,satlon,alt,dn,Ne_scaling,testing,f107=f107, f107a=f107a, f107p=f107p, apmsis=apmsis)
...
# ICON FUV WGS84
def calculate_pixel_1356_nighttime_WGS84(ze,az,satlat,satlon,satalt,dn,symmetry = 0,cont=1,Ne_scaling=1., step=10.,testing = 0,total_distance = 5000.,f107=None, f107a=None, f107p=None, apmsis=None):
    '''
    Step along a desired look direction from a given observing location and
    ...
    for j in range(0,np.size(latlonalt,1)):
        ...
        if symmetry == 0:
            VER,MN,Ne,_ = calc_1356_nighttime(satlat,satlon,latlonalt[2,j],dn,Ne_scaling,testing,f107=f107, f107a=f107a, f107p=f107p, apmsis=apmsis)
        elif symmetry == 1:
            VER,MN,Ne,_ = calc_1356_nighttime(latlonalt[0,j],latlonalt[1,j],latlonalt[2,j],dn,Ne_scaling,testing,f107=f107, f107a=f107a, f107p=f107p, apmsis=apmsis)
...
# ICON FUV
def get_Photons_from_Brightness_1356_nighttime(ze,az,satlat,satlon,satalt,dn,symmetry =0,shperical=1,exposure=0.,testing = 0,cont=1,TE=0.,Ne_scaling = 1.,step = 10, total_distance = 5000., stripes_used = 0, f107=None, f107a=None, f107p=None, apmsis=None):
    '''
    ...
    if shperical == 0:
        Brightness = calculate_pixel_1356_nighttime(ze,az,satlat,satlon,satalt,dn,cont,Ne_scaling,step,testing,f107=f107, f107a=f107a, f107p=f107p, apmsis=apmsis)
    elif shperical == 1:
        Brightness = calculate_pixel_1356_nighttime_WGS84(ze,az,satlat,satlon,satalt,dn,symmetry,cont,Ne_scaling,step,testing,total_distance,f107=f107, f107a=f107a, f107p=f107p, apmsis=apmsis)
...
# ICON FUV
def get_Photons_from_Brightness_Profile_1356_nighttime(ze,az,satlat,satlon,satalt,dn,symmetry=0,shperical=1,exposure=0.,testing = 0,cont=1,TE=0.,Ne_scaling = 1.,step = 10,total_distance = 5000.,stripes_used = 0,proc=16, f107=None, f107a=None, f107p=None, apmsis=None):
    '''
    ...
    try:
        ...
        job_args = [(ze[i],az[i] ,satlat,satlon,satalt, dn,symmetry,shperical,exposure,testing,cont,TE,Ne_scaling,step,total_distance,stripes_used,f107,f107a,f107p,apmsis) for i in range(0,len(ze))]
