# Ulas Kamaci - 2022-04-26
# uncertainty_calculation v3.2
...
# v3.2 - 2022-04-26: Fix the undefined indnonzero error which occurs when
# calculated backgrounds are provided as input. Ulas Kamaci.
...
def uncertainty_calculator(
    ...
    if background_mean is not None:
        ...
        background_mean = np.squeeze(background_mean)
        # if a single altitude profile is provided, copy it along the time indices
        if background_mean.size == profile_raw.size:
            indnonzero = np.arange(background_mean.shape[1])
        elif len(background_mean.shape) == 2:
            if not np.allclose(background_mean[background_mean.shape[0]//2], np.zeros(256)):
                # this happens when there are multiple turret angles. in this case
                # take the middle entry, which should correspond to turret angle=0.
                background_mean = background_mean[background_mean.shape[0]//2]
            else:
                for i in range(background_mean.shape[0]):
                    if np.allclose(background_mean[i], np.zeros(256)):
                        background_mean[i] = np.nan * np.ones(256)
                background_mean = np.nanmedian(background_mean, axis=0)
        if len(background_mean.shape) == 1:
            # perform median filter based outlier detection to correct the standard
            # deviation for constant pixels
            bkgmed_mean = median_filter(background_mean, size=50)
            bkgmed_std = median_filter(background_std, size=50)
            indzero = np.where((bkgmed_mean-background_mean)/abs(bkgmed_mean) > 0.8)[0]
            indnonzero = np.where((bkgmed_mean-background_mean)/abs(bkgmed_mean) <= 0.8)[0]

            background_std[indzero] = bkgmed_std[indzero]
