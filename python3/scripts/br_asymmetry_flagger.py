...
# Contrary to the original asymmetry_flagger, this one does not use any external
# TEC data to estimate asymmetries. it primarily uses the variablility in the 
# peak brightness altitude - sudden rises and drops - to estimate asymmetries.
...
def flagger(file_l1=None, file_l2=None, stripe=2, png_stub=None, plot=False):
        ...
        for orbit in np.unique(orbit_list):
            try:
                ...
                if asymm_check.max() >=5 :
                    asymm_center = np.argmax(np.convolve(
                    asymm_center_int,np.hamming(13), 'valid'))
                ...
                flag_counter += sum(asymm_flag)
                qual1_counter += sum(qual==1)
                flagqual1_locations = asymm_flag & (qual==1)
                ...
                if plot==True and sum(flagqual1_locations)>0:
                    ...
                    axes[2].plot(brmax, '-o', color='blue', label='Peak Br')
                    ax2 = axes[2].twinx()
                    ax2.plot(brmax_alt, '-o', color='purple', label='Peak Br Alt')
                    ax2.set_ylabel('Altitude')
                    ax2.set_ylim((150,350))
                    axes[2].set_title('Peak Br / Peak Br Alt / Satlat vs Epoch')
                    axes[2].set_xlabel('Epoch')
                    axes[2].set_ylabel('Peak Br')
    ...
    return  nighttime_counter, flag_counter, qual1_counter, qual1flag_counter, hmf2s_all, hmf2s_flagged
...
if __name__=='__main__':
    ...
    files_l2.sort()
    stripe=2
    path_fig = path_dir + 'figures/aurora_flagging/asymmetry_flag_br100_alt200'
    ...
    for file_l1,file_l2 in tqdm(zip(files_l1[62:93], files_l2[62:93])):
        (nighttime_ctr, 
        flag_ctr, 
        qual1_ctr,
        qual1flag_ctr,
